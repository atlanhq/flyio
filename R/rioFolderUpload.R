...
#' rioFileUpload("~/Downloads/", "tests/")
#' }

rioFolderUpload <- function(localfolder, bucketpath, pattern = "*", overwrite = TRUE, data_source = rioGetDataSource(),
  ...
  if(!isTRUE(overwrite)){
    ...
    bucketfiles = rioListFiles(path = bucketpath, pattern = pattern, recursive = T,
